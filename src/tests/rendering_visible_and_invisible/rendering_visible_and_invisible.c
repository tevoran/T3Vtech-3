...
int main(int argc, char *argv[])
{
	...
	for(int i=0; i<num; i++)
	{
		...
		tt_vec3 pos={-30.0f + 15.0f * i, 0.0f, -40.0f};
		tt_3d_object_set_position(obj[i], &pos);
		tt_3d_object_light_affected(obj[i], true);
		tt_3d_object_add_point_light(obj[i], light);
		if(i==2)
		{
		...
		{	
			tt_3d_object_make_invisible(obj[i], false);			
		}
	}
